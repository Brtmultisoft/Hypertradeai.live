...
const LevelRoiHistory = () => {
  ...
  const processResponse = (response) => {
    ...
    if (response.data.status) {
      ...
      if (response.data.result && response.data.result.list) {
        ...
        // Filter for level_roi_income type if needed
        const filteredList = response.data.result.list.filter(income =>
          ...
          (!filterLevel || income.level === filterLevel)
        );
        ...
        // Filter the array for level ROI income
  ...
  // Fetch level ROI income data
  const fetchLevelRoiIncomes = async () => {
    setLoading(true);
    setError(null);
    try {
      ...
      try {
        const response = await axios.get(`${API_URL}/admin/get-all-incomes`, {
          ...
          params: {
            ...
            type: "level_roi_income", // Filter by level_roi_income type
            level: filterLevel,
  ...
  }, [page, rowsPerPage, sortField, sortDirection, filterLevel]);
