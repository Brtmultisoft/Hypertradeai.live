/**
 * Script to process specific users who didn't receive their daily profits
 * 
 * This script will:
 * 1. Process the 5 specific users who didn't receive their daily profits
 * 2. Calculate and distribute their profits
 * 3. Update their trade activation records
 */
...
// List of activation IDs that need to be processed
const activationIds = [
  "6829686bdd04e29b3cc83b3f", // sarabjeet01461@gmail.com
  "68297909dd04e29b3cc83ce5", // gautam.papneja@gmail.com
  "68298401dd04e29b3cc83ef9", // mukulbatra143@gmail.com
  "68298564dd04e29b3cc840fa", // narulaannu09@gmail.com
  "68299baedd04e29b3cc8422c"  // nk1023509@gmail.com
];

async function processSpecificUsers() {
  let client;
  ...
  try {
    ...
    for (const activationId of activationIds) {
      try {
        ...
        const activation = await db.collection('tradeactivations').findOne({
          _id: new ObjectId(activationId)
        });

        if (!activation) {
          console.log(`Activation not found: ${activationId}`);
          errors.push({
        ...
        const user = await db.collection('users').findOne({
          _id: new ObjectId(activation.user_id)
        });

        if (!user) {
          console.log(`User not found for activation ${activationId}`);
          errors.push({
            activation_id: activationId,
        ...
        for (const investment of activeInvestments) {
          ...
          const investmentPlan = await db.collection('investmentplans').findOne({
            _id: new ObjectId(investment.investment_plan_id)
          });
            ...
            { _id: new ObjectId(activationId) },
            {
              $set: {
                ...
                profit_details: {
                  ...
                  profit_rate: roiRate,
                  income_id: incomeResult.insertedId.toString()
                },
                cron_execution_id: cronExecutionId,
                updated_at: new Date()
              }
        ...
        console.error(`Error processing activation ${activationId}:`, error);
        errors.push({
          activation_id: activationId,
          error: error.message
        });
    ...
    await db.collection('cronexecutions').updateOne(
      { _id: cronExecutionId },
      {
        $set: {
          ...
          execution_details: {
            total_activations: activationIds.length,
            processed_count: processedCount,
            skipped_count: activationIds.length - processedCount
          },
          updated_at: new Date()
        }
      }
    ...
    console.log('\nSummary:');
    console.log(`Total activations processed: ${activationIds.length}`);
