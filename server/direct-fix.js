/**
 * Script to directly fix trade activations in the database
 *
 * This script uses the MongoDB driver directly to update the records
 */
...
async function directFix() {
  ...
  try {
    ...
    for (const activation of pendingActivations) {
      try {
        // Get user data
        const user = await db.collection('users').findOne({ _id: activation.user_id });
        if (!user) {
        ...
        for (const investment of activeInvestments) {
          ...
          const investmentPlan = await db.collection('investmentplans').findOne({ _id: investment.investment_plan_id });
